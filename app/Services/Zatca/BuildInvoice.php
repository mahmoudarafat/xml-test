...
//        $setting = $this->CI->db->where('id',$this->company_setting->id)->get('settings')->row();
        $setting = DB::table('zatca_settings')->first();
    ...
    public function GetPIH(){
        ...
        $previous_invoice = SaleProcess::where($this->invoice->invoice_identifier,$this->invoice->invoice_counter)->where('shop_id',auth()->user()->shop_id )->first();
